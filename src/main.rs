...
/// Note this differs from the generic PWM_SET_DUTY command â fan duty takes only the
...
fn set_fan_duty(duty_percent: u8) -> Result<(), EcError> {
    ...
    // `percent` is a 32-bit little-endian value, not a single byte â send all four
...
impl FanRgbApp {
    fn new() -> Self {
        ...
        Self {
            start_key: 0,
            // 0 means "no fan choice made yet" â the fan is only written once the
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.toggle_button("50%", self.fan_duty == 50) {
                    self.fan_duty = 50;
                }
                if ui.toggle_button("100%", self.fan_duty == 100) {
                    self.fan_duty = 100;
                }
            });
